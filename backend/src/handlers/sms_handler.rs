...
use crate::db::{deposit_repo, user_repo, DbPool};
use crate::models::{loan::Loan, transaction::Transaction, user::User};
use crate::services::sms_parser::{self, SmsCommand};
use crate::services::{
    africas_talking_client::AfricasTalkingClient, loan_service, stellar_client::StellarClient,
};
use crate::utils::{currency_converter, phone_formatter};
...
pub async fn handle_sms(
    ...
    // NOT JSON
    payload: web::Form<AfricaTalkingWebhook>,
) -> impl Responder {
    let payload = payload.into_inner();

    info!("Incoming Africa's Talking webhook: {:?}", payload);

    let raw_phone = &payload.from;
    let formatted_phone = phone_formatter::format_phone(raw_phone);
    let msg_body = &payload.text;

    info!("Parsed incoming SMS from {}: {}", formatted_phone, msg_body);

    let parsed_cmd = sms_parser::parse_message(msg_body);

    let stellar_client = StellarClient::new(settings.get_ref());
    let africas_talking = AfricasTalkingClient::new(settings.get_ref());

    let reply_msg = match parsed_cmd {
        SmsCommand::Register(name) => {
            match user_repo::get_user(pool.get_ref(), &formatted_phone).await {
                Ok(Some(_)) => {
                    "PesaText: You are already registered under this phone number."
                        .to_string()
                }
 ...
 main
                            ...
                            let _ = deposit_repo::create_transaction(
                                pool.get_ref(),
                                &tx,
                            )
                            .await;
...
main
                ...
                Err(_) => {
                    "PesaText: Database error occurred.".to_string()
                }
            }
        }
        ...
        SmsCommand::Balance => {
            match user_repo::get_user(pool.get_ref(), &formatted_phone).await {
                Ok(Some(_)) => {
                    let bal_res =
                        stellar_client.get_balance(&formatted_phone).await;

                    let loan_res =
                        stellar_client.get_loan_due(&formatted_phone).await;
 ...
 backend/africas-talking-sms
                            ...
                            let bal_kes =
                                currency_converter::xlm_to_kes(bal_xlm);
 ...
 main
                        ...
                        _ => {
                            "PesaText: Failed to retrieve balance from Stellar blockchain."
                                .to_string()
                        }
                    }
                }
                ...
                Ok(None) => {
                    "PesaText: You are not registered. Reply with REGISTER [Name] to create an account."
                        .to_string()
                }

                Err(_) => {
                    "PesaText: System error occurred.".to_string()
                }
            }
        }
 ...
 main
                ...
                Ok(None) => {
                    "PesaText: Registration required. Text REGISTER [Name] to open an account."
                        .to_string()
                }

                Err(_) => {
                    "PesaText: Database save error.".to_string()
                }
            }
        }
        ...
        SmsCommand::Borrow(kes_amount) => {
            match user_repo::get_user(pool.get_ref(), &formatted_phone).await {
                Ok(Some(_)) => {
                    match deposit_repo::get_active_loan(
                        pool.get_ref(),
                        &formatted_phone,
                    )
                    .await
                    {
                        Ok(Some(active_loan)) => {
                            let loan_xlm =
                                currency_converter::stroops_to_xlm(
                                    (
                                        active_loan.principal_stroops
                                            + active_loan.interest_stroops
                                    )
                                        .into(),
                                );
...
main
                                    let new_loan = Loan {
                                        ...
                                        principal_stroops:
                                            calc.principal_stroops as i64,
                                        interest_stroops:
                                            calc.interest_stroops as i64,
                                    ...
                                    let _ =
                                        deposit_repo::create_transaction(
                                            pool.get_ref(),
                                            &tx,
                                        )
                                        .await;
                                    ...
                                    let due_kes =
                                        currency_converter::xlm_to_kes(
                                            currency_converter::stroops_to_xlm(
                                                calc.total_due_stroops.into(),
                                            ),
                                        );

                                    format!(
                                        "PesaText Loan: Approved! {:.2} KES (~{:.2} XLM) has been credited to your balance. Total due in 30 days: {:.2} KES (5% interest).",
                                        kes_amount,
                                        requested_xlm,
                                        due_kes,
                                    )
 ...
 main
                        ...
                        Err(_) => {
                            "PesaText: Database verification failed."
                                .to_string()
                        }
                    }
                }
                ...
                Ok(None) => {
                    "PesaText: Registration required first. Text REGISTER [Name]."
                        .to_string()
                }

                Err(_) => {
                    "PesaText: Database query error.".to_string()
                }
            }
        }
 ...
 backend/africas-talking-sms
                            ...
                            match stellar_client
                                .get_balance(&formatted_phone)
                                .await
                            {
                                Ok(balance)
                                    if balance >= repay_stroops =>
                                {
                                    match stellar_client
                                        .repay(
                                            &formatted_phone,
                                    ...
                                    {
                                        Ok(res) => {
                                            let remaining_due_stroops =
                                                res.loan_due_stroops;

                                            if remaining_due_stroops == 0 {
                                                let _ =
                                                    deposit_repo::update_loan_status(
                                                        pool.get_ref(),
 ...
 main
                                            ...
                                            let remaining_kes =
                                                currency_converter::xlm_to_kes(
                                                    currency_converter::stroops_to_xlm(
                                                        remaining_due_stroops,
                                                    ),
                                                );

                                            format!(
                                                "PesaText Repayment: Repaid {:.2} KES (~{:.2} XLM). Outstanding due: {:.2} KES.",
                                                kes_amount,
                                                repay_xlm,
                                                remaining_kes,
                                            )
 ...
 main
                                ...
                                _ => {
                                    "PesaText: Insufficient savings balance."
                                        .to_string()
                                }
                            }
                        }
                        ...
                        Ok(None) => {
                            "PesaText Repay: No active loan found."
                                .to_string()
                        }

                        Err(_) => {
                            "PesaText: Database lookup failed."
                                .to_string()
                        }
                    }
                }
                ...
                Ok(None) => {
                    "PesaText: You are not registered.".to_string()
                }
        ...
        SmsCommand::Unknown(msg) => {
            format!(
                "PesaText: {} Send HELP for commands list.",
                msg
            )
        }
    };

...
use crate::services::{loan_service, stellar_client::StellarClient};
...
pub async fn handle_sms(
                ...
                Ok(None) => {
                    match stellar_client.register(&formatted_phone, &name).await {
                        Ok(res) => {
                            ...
                            format!(
                                "PesaText: Registration successful! Welcome, {}. Send HELP to see options.",
                                name
                            )
                        }
        ...
        SmsCommand::Save(kes_amount) => {
            match user_repo::get_user(pool.get_ref(), &formatted_phone).await {
                Ok(Some(_)) => {
                    ...
                    let mock_mpesa_ref = format!("MP{}", &payload.id[0..8].to_uppercase());
    ...
    info!("Sending response: {}", reply_msg);
